...
controller.home = asyncHandler(async (req, res, next) => {
  const shoplist = await seqlz.query("select c.id as id, c.name as cname, g.name as gname, sum(l.quant * le.quant_unit - le.quant) as in_stock, cs.name as case_name from locations as l, groups as g, components as c, location_entry as le, cases as cs where g.id = c.group_id and l.id = le.location_id and l.quant > 0 and c.id = le.component_id and c.case_id = cs.id group by c.id, c.name, g.name, cs.name order by gname, cname, case_name",
    {
